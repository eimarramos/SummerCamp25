...
static void Ejemplo6()
{
    ...
    List<string> resultadoDepartamento = resultadoMetodos
        ...
        .ThenBy(empleado => empleado.Apellidos)
...
static void Ejemplo5()
{
    ...
    var nivelesMayores6LinqMetodos = niveles
        .Where(n => n.Length > 6)
        .OrderBy(n => n)
        .Select(n => n);
}
...
static void Ejemplo3()
{
    ...
    procesos.Add(e =>
    {
        if (e.AntiguedadAnios > 5)
        {
            e.EdadEstimada = DateTime.Now.Year + 1;
        }
        else
        {
            e.EdadEstimada = DateTime.Now.Year + 3;
        }
        return e;
    });
    ...
    procesos.Add(e =>
    {
        e.RiesgoRotacion = e.ClasificacionSalarial == "BAJO" && e.AntiguedadAnios > 5;
        return e;
    });
    ...
    procesos.Add(e =>
    {
        if (e.SalarioAnual < 30000)
            e.RetencionIRPF = 0.15m;
        else if (e.SalarioAnual <= 50000)
            e.RetencionIRPF = 0.20m;
        else
            e.RetencionIRPF = 0.25m;
        e.SalarioAnual -= e.SalarioAnual * e.RetencionIRPF;
        return e;
    });
    ...
    procesos.Add(e =>
    {
        ...
        {
            e.FechaRevisionContrato = DateTime.Now.AddMonths(6);
        }
        e.FechaRevisionContrato = DateTime.Now.AddYears(1);
    ...
    foreach (var empleado in empleados)
    {
        ...
        Console.WriteLine($"Fecha estimada de ascenso: {empleado.EdadEstimada}");
        Console.WriteLine($"Riesgo de rotación: {(empleado.RiesgoRotacion ? "ALTO" : "BAJO")}");
